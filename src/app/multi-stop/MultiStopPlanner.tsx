  ...
  return (
    <div className="mt-4 space-y-6">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 md:p-6">
        <h2 className="text-lg font-bold text-slate-900">Build the trip</h2>
        <p className="text-xs text-slate-500">
          {isFallback
            ? "Using Bangalore centre — grant location for routes from where you are."
            : "Starting from your live location."}
        </p>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3">
          <button
            type="button"
            onClick={run}
            disabled={isPlanning || status === "prompting"}
          ...
          <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <Stat
                ...
                label="Stops"
                value={plan.stops.length.toString()}
                sub={`from ${plan.candidatesConsidered} candidates`}
              />
              <Stat
                icon={<Navigation className="h-4 w-4" />}
                label="Distance"
                value={formatKm(plan.totals.distanceKm)}
                sub="round trip via OSRM"
              />
              <Stat
                icon={<Clock className="h-4 w-4" />}
                label="Driving time"
                value={formatMinutes(plan.totals.durationMinutes)}
                sub="excluding visit time"
              />
              <Stat
                icon={<Wallet className="h-4 w-4" />}
                label="Total cost"
                value={formatINR(plan.totals.cost)}
                sub={`${formatINR(plan.totals.perPersonCost)} per person`}
              />
            </div>
            <p className="mt-3 text-xs text-emerald-900/70">
              Unspent: {formatINR(plan.totals.unspentBudget)} budget ·{" "}
              {formatMinutes(plan.totals.unspentMinutes)} time · Live OSM ·{" "}
              {plan.overpassPlaces} OSM places considered, {plan.seedPlaces} curated.
              {plan.overpassError && (
                <span className="ml-1 text-amber-700">
                  (Overpass error: {plan.overpassError}, used curated picks only)
                </span>
              )}
            </p>
          </section>
            ...
            <section>
              <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-600 px-4 py-2 text-sm font-bold text-white shadow-lg shadow-emerald-500/30 transition hover:from-emerald-600 hover:to-emerald-700"
                >
              ...
              <p className="mt-2 text-xs text-slate-500">
                Numbered pins are stops in optimal visit order. Green = your
                location. The polyline follows real driving roads (OSRM); if a
                stop appears off the line, OSRM couldn&apos;t reach it via car.
              </p>
            </section>
          )}

          <section>
            <h2 className="mb-3 text-lg font-bold text-slate-900">Itinerary</h2>
              ...
              {plan.stops.map((s, i) => (
                <li key={s.id} className="relative rounded-xl border border-slate-200 bg-white p-4">
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      <p className="text-sm text-slate-500">Stop {i + 1}</p>
                    ...
                    <div className="text-right text-xs text-slate-600">
                      <p>
                        {formatKm(s.arrivalKmFromPrev)} ·{" "}
                        {formatMinutes(s.arrivalMinutesFromPrev)} from{" "}
                        {i === 0 ? "start" : "previous stop"}
                      </p>
                      <p className="font-semibold text-slate-900">
                        Stay {formatMinutes(s.idealMinutes)}
                      </p>
                    </div>
                  </div>
                  <div className="mt-2 flex flex-wrap gap-2 text-xs">
                    ...
                    {s.travelCost > 0 && (
                      <Chip>Fuel {formatINR(s.travelCost)}</Chip>
                    )}
                    {s.meta?.citySeedSlug && (
                      <a
                    ...
                    {s.meta?.osmId && (
                      <a
                        href={`https://www.openstreetmap.org/${s.meta.osmId}`}
                        target="_blank"
